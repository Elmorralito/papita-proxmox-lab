"""Structured JSON logging to stderr for the pfSense MCP process.

Configures the ``pfsense_mcp`` logger hierarchy to emit one JSON object per line on
``stderr``, leaving ``stdout`` free for MCP stdio protocol traffic. CLI entry points
use :func:`configure_cli_logging` for plain-text results on stdout.
"""
...
ENV_LOG_LEVEL = "PFSENSE_LOG_LEVEL"
PACKAGE_LOGGER = "pfsense_mcp"
CLI_LOGGER = "pfsense_mcp.cli"
...
def configure_logging(level: str | int | None = None) -> None:
    """Configure the ``pfsense_mcp`` logger to write JSON lines to stderr."""
    if level is None:
